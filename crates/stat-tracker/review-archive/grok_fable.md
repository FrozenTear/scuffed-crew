# Stat Tracker — Merged Deep Review (Grok + Fable)

**Date:** 2026-07-09 · **Branch:** `stat-tracker/add-neon-junction-aatlis` · **Scope:** `crates/stat-tracker` (+ server sync surface)

**Sources:**
- `fable_review.md` — Claude Fable 5, four parallel deep-dives (capture/OCR, GUI/daemon, correctness, architecture), Serena-verified against function bodies
- `grok_review.md` — Grok multi-agent deep dive (OCR/preprocess, capture/detect, storage/sync, GUI/correctness, end-to-end pipeline)

**Method of this doc:** Deduplicate, re-rank, and reconcile both reviews. Prefer Fable on data-path / state-machine / sync correctness (including cases where Grok graded intentional design that is actually wrong). Prefer Grok on GUI lifecycle, ops safety, and product footguns. Shared findings are stated once at the highest justified severity.
...
|-----------|-------|---------|
| Domain / session correctness | **A−** | Grace windows, trust gates, majority repair — battle-tested; residual M1/M5 holes |
...
4. **Closing residual wrong-data bugs** (unfinished sessions, map-vote, Ana/Havana, portrait geometry).
...
*Merged from `fable_review.md` and `grok_review.md` for a single implementation backlog. Prefer this file when prioritizing work; consult the source reviews for longer rationale and raw agent notes.*
