# Tag `stat-tracker-v0.4.5` (Robert authorized 2026-09-04)

Operator runbook; it is not the GitHub Release body. Robert authorized
the **full 0.4.5 cut**: bump, merge the release PR, annotated tag, and
`git push origin stat-tracker-v0.4.5`.

Proposed tag: **`stat-tracker-v0.4.5`**
...
This ships three feature PRs already on `main`:

- #70 compact Settings cards / field widths (Companion hotkey setting unchanged)
- #71 reuse unfinished same-map/hero instead of a second empty Games card
  (Tab debounce and 1800s session grouping unchanged)
- #72 compact Maps cards (2–4 columns, WR bar, win/loss stripe)
...
pattern as v0.4.4 (`docs/notes/stat-tracker-v0.4.4-tag.md`).

## 0. Confirm

- [x] Robert has said **yes** to tagging `stat-tracker-v0.4.5`
- [x] Robert has said **yes** to merging the release PR into `main`
- [ ] Release PR is against `main` and CI is green
- [ ] `git ls-remote --tags origin 'stat-tracker-v0.4.5'` is empty

## 1. Merge the release PR

Merge via GitHub when CI is green. Match #69 / #70 / #71 / #72: a merge
commit (`Merge pull request #N from FrozenTear/<branch>`), not squash/rebase.

```sh
git fetch origin main
git log -1 --oneline origin/main
# expect the merge commit of the v0.4.5 bump PR
```
...
git tag -a stat-tracker-v0.4.5 "$SHA" -m "Stat Tracker v0.4.5"
git show --no-patch stat-tracker-v0.4.5
```

Check that the tagged tree has `crates/stat-tracker` **0.4.5**:

```sh
git show stat-tracker-v0.4.5:crates/stat-tracker/Cargo.toml | head -6
# version = "0.4.5"
```

If `stat-tracker-v0.4.5` already exists locally or on `origin`, **stop**.
...
- Release: https://github.com/FrozenTear/scuffed-crew/releases/tag/stat-tracker-v0.4.5

The job injects `SST_RELEASE_VERSION=0.4.5` so
`scuffed-stat-tracker --version` is `scuffed-stat-tracker 0.4.5`.

Release notes are `crates/stat-tracker/CHANGELOG.md` section `## 0.4.5`, then
the usual git-log since the previous `stat-tracker-v*` tag (0.4.4), then
...
scuffed-stat-tracker --version   # expect: scuffed-stat-tracker 0.4.5
# Settings: compact cards / capped field widths; Companion hotkey unchanged.
# Games: unfinished same-map/hero within ~20 min reuses (no second empty card).
# Maps: compact 2–4 column cards with WR bar and win/loss stripe.
```
...
- Do **not** `gh release create` by hand — the workflow owns the tarball,
  sha256, and notes.
- Do **not** push `stat-tracker-v0.4.5` onto a commit that is not on `main`
  after the merge.
