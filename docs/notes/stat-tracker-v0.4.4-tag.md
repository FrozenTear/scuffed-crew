# Tag `stat-tracker-v0.4.4` (Robert authorized 2026-09-03)

Operator runbook; it is not the GitHub Release body. Robert authorized
the **full 0.4.4 cut**: bump, merge the release PR, annotated tag, and
`git push origin stat-tracker-v0.4.4`.

Proposed tag: **`stat-tracker-v0.4.4`**
...
This ships the Companion overlay show/hide hotkey already on `main` via
#68: Settings → Companion, default Super+Shift+C, evdev/Wayland (same
`input` group / seat `uaccess` as Tab). OverlayHold matches the tray.
...
pattern as v0.4.3 (`docs/notes/stat-tracker-v0.4.3-tag.md`).

## 0. Confirm

- [x] Robert has said **yes** to tagging `stat-tracker-v0.4.4`
- [x] Robert has said **yes** to merging the release PR into `main`
- [ ] Release PR is against `main` and CI is green
- [ ] `git ls-remote --tags origin 'stat-tracker-v0.4.4'` is empty

## 1. Merge the release PR

Merge via GitHub when CI is green. Match #66 / #67 / #68: a merge commit
(`Merge pull request #N from FrozenTear/<branch>`), not squash/rebase.

```sh
git fetch origin main
git log -1 --oneline origin/main
# expect the merge commit of the v0.4.4 bump PR
```
...
git tag -a stat-tracker-v0.4.4 "$SHA" -m "Stat Tracker v0.4.4"
git show --no-patch stat-tracker-v0.4.4
```

Check that the tagged tree has `crates/stat-tracker` **0.4.4**:

```sh
git show stat-tracker-v0.4.4:crates/stat-tracker/Cargo.toml | head -6
# version = "0.4.4"
```

If `stat-tracker-v0.4.4` already exists locally or on `origin`, **stop**.
...
- Release: https://github.com/FrozenTear/scuffed-crew/releases/tag/stat-tracker-v0.4.4

The job injects `SST_RELEASE_VERSION=0.4.4` so
`scuffed-stat-tracker --version` is `scuffed-stat-tracker 0.4.4`.

Release notes are `crates/stat-tracker/CHANGELOG.md` section `## 0.4.4`, then
the usual git-log since the previous `stat-tracker-v*` tag (0.4.3), then
...
scuffed-stat-tracker --version   # expect: scuffed-stat-tracker 0.4.4
# Overlay hotkey: Settings → Companion, default Super+Shift+C.
# Needs input group or seat uaccess (same as Tab).
```
...
- Do **not** `gh release create` by hand — the workflow owns the tarball,
  sha256, and notes.
- Do **not** push `stat-tracker-v0.4.4` onto a commit that is not on `main`
  after the merge.
