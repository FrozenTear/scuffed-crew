# Tag `stat-tracker-v0.4.9` (author tags after merge)

Operator runbook; it is not the GitHub Release body. This bump PR
covers #80 + #81 already on `main`. Author will tag
`stat-tracker-v0.4.9` after merge. Do not create the tag from this PR.
Merge + tag only if CI is fully green. Do not merge or tag if this
bump PR is not fully green.

Proposed tag: **`stat-tracker-v0.4.9`**
...
- #80 On-hit poll Victory/Defeat debug PNGs (when debug OCR is on,
  the normal poll path saves confirm / first-streak frames under
  `debug/poll/`; mid-match ticks stay silent)
- #81 POTG / end-reel slow-mode wake (Play of the Game or highlight
  end-reel holds full ~4s poll cadence for 45s so the short
  Victory/Defeat window is sampled; confirm rules unchanged)

In-app Update now (0.4.8), Settings Maps-level polish (0.4.7),
...
pattern as v0.4.8 (`docs/notes/stat-tracker-v0.4.8-tag.md`).

## 0. Confirm

- [ ] Author will tag `stat-tracker-v0.4.9` after this PR merges
- [ ] Author will merge the release PR into `main`
- [ ] Release PR is against `main` and CI is green
- [ ] `git ls-remote --tags origin 'stat-tracker-v0.4.9'` is empty

## 1. Merge the release PR

Merge via GitHub when CI is green. Match #79 / #80 / #81: a merge
commit (`Merge pull request #N from FrozenTear/<branch>`), not squash/rebase.

```sh
git fetch origin main
git log -1 --oneline origin/main
# expect the merge commit of the v0.4.9 bump PR
```
...
git tag -a stat-tracker-v0.4.9 "$SHA" -m "Stat Tracker v0.4.9"
git show --no-patch stat-tracker-v0.4.9
```

Check that the tagged tree has `crates/stat-tracker` **0.4.9**:

```sh
git show stat-tracker-v0.4.9:crates/stat-tracker/Cargo.toml | head -6
# version = "0.4.9"
```

If `stat-tracker-v0.4.9` already exists locally or on `origin`, **stop**.
Do not force-push or retag.

## 3. Push the tag (triggers artifacts)

```sh
git push origin stat-tracker-v0.4.9
```

Watch:

- Actions: https://github.com/FrozenTear/scuffed-crew/actions/workflows/stat-tracker-release.yml
- Release: https://github.com/FrozenTear/scuffed-crew/releases/tag/stat-tracker-v0.4.9

The job injects `SST_RELEASE_VERSION=0.4.9` so
`scuffed-stat-tracker --version` is `scuffed-stat-tracker 0.4.9`.

Release notes are `crates/stat-tracker/CHANGELOG.md` section `## 0.4.9`, then
the usual git-log since the previous `stat-tracker-v*` tag (0.4.8), then
...
scuffed-stat-tracker --version   # expect: scuffed-stat-tracker 0.4.9
# Debug OCR on: poll confirm / first-streak frames under debug/poll/
# POTG / end-reel wakes slow poll to full ~4s cadence for 45s
```
...
- Do **not** `gh release create` by hand — the workflow owns the tarball,
  sha256, and notes.
- Do **not** push `stat-tracker-v0.4.9` onto a commit that is not on `main`
  after the merge.
