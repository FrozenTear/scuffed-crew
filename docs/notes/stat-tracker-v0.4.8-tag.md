# Tag `stat-tracker-v0.4.8` (Robert authorized 2026-09-04)
...
annotated tag, and `git push origin stat-tracker-v0.4.8`. Do not merge or
tag if this bump PR is not fully green.

Proposed tag: **`stat-tracker-v0.4.8`**
...
- #78 In-app Update now + Wayland Copy for the install curl (preflight
  then bootstrap with pinned `STAT_TRACKER_TAG`; clear blocked reasons;
  Wayland prefers `wl-copy`, toast if wl-clipboard is missing)

Settings Maps-level polish (0.4.7), Maps-grammar Seasons grid (0.4.6),
...
pattern as v0.4.7 (`docs/notes/stat-tracker-v0.4.7-tag.md`).

## 0. Confirm

- [x] Robert has said **yes** to tagging `stat-tracker-v0.4.8`
- [x] Robert has said **yes** to merging the release PR into `main`
- [ ] Release PR is against `main` and CI is green
- [ ] `git ls-remote --tags origin 'stat-tracker-v0.4.8'` is empty

## 1. Merge the release PR

Merge via GitHub when CI is green. Match #77 / #78: a merge
commit (`Merge pull request #N from FrozenTear/<branch>`), not squash/rebase.

```sh
git fetch origin main
git log -1 --oneline origin/main
# expect the merge commit of the v0.4.8 bump PR
```
...
git tag -a stat-tracker-v0.4.8 "$SHA" -m "Stat Tracker v0.4.8"
git show --no-patch stat-tracker-v0.4.8
```

Check that the tagged tree has `crates/stat-tracker` **0.4.8**:

```sh
git show stat-tracker-v0.4.8:crates/stat-tracker/Cargo.toml | head -6
# version = "0.4.8"
```

If `stat-tracker-v0.4.8` already exists locally or on `origin`, **stop**.
Do not force-push or retag.

## 3. Push the tag (triggers artifacts)

```sh
git push origin stat-tracker-v0.4.8
```

Watch:

- Actions: https://github.com/FrozenTear/scuffed-crew/actions/workflows/stat-tracker-release.yml
- Release: https://github.com/FrozenTear/scuffed-crew/releases/tag/stat-tracker-v0.4.8

The job injects `SST_RELEASE_VERSION=0.4.8` so
`scuffed-stat-tracker --version` is `scuffed-stat-tracker 0.4.8`.

Release notes are `crates/stat-tracker/CHANGELOG.md` section `## 0.4.8`, then
the usual git-log since the previous `stat-tracker-v*` tag (0.4.7), then
...
scuffed-stat-tracker --version   # expect: scuffed-stat-tracker 0.4.8
# Settings: Update now (preflight then pinned-tag bootstrap); Copy command
# prefers wl-copy on Wayland; toast if wl-clipboard is missing.
```
...
- Do **not** `gh release create` by hand — the workflow owns the tarball,
  sha256, and notes.
- Do **not** push `stat-tracker-v0.4.8` onto a commit that is not on `main`
  after the merge.
