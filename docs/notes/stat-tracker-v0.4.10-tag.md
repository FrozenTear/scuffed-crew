# Tag `stat-tracker-v0.4.10` (author tags after merge)

Operator runbook; it is not the GitHub Release body. This bump PR
covers #83 already on `main`. Author will tag
`stat-tracker-v0.4.10` after merge. Do not create the tag from this PR.
Merge + tag only if CI is fully green. Do not merge or tag if this
bump PR is not fully green.

Proposed tag: **`stat-tracker-v0.4.10`**
...
- #83 POTG nameplate title-card wake (Play of the Game **nameplate**
  title card — gold battletag + white title, color gate — holds full
  ~4s poll cadence for 45s; letterbox / highlight end-reel path
  unchanged; phrase OCR optional / log only; calibrated on a real
  2560×1440 card)

0.4.9 already shipped #80 (on-hit poll debug PNGs) and #81 (POTG /
end-reel letterbox wake). In-app Update now (0.4.8), Settings
...
pattern as v0.4.9 (`docs/notes/stat-tracker-v0.4.9-tag.md`).

## 0. Confirm

- [ ] Author will tag `stat-tracker-v0.4.10` after this PR merges
- [ ] Author will merge the release PR into `main`
- [ ] Release PR is against `main` and CI is green
- [ ] `git ls-remote --tags origin 'stat-tracker-v0.4.10'` is empty

## 1. Merge the release PR

Merge via GitHub when CI is green. Match #80 / #81 / #82 / #83: a merge
commit (`Merge pull request #N from FrozenTear/<branch>`), not squash/rebase.

```sh
git fetch origin main
git log -1 --oneline origin/main
# expect the merge commit of the v0.4.10 bump PR
```
...
git tag -a stat-tracker-v0.4.10 "$SHA" -m "Stat Tracker v0.4.10"
git show --no-patch stat-tracker-v0.4.10
```

Check that the tagged tree has `crates/stat-tracker` **0.4.10**:

```sh
git show stat-tracker-v0.4.10:crates/stat-tracker/Cargo.toml | head -6
# version = "0.4.10"
```

If `stat-tracker-v0.4.10` already exists locally or on `origin`, **stop**.
Do not force-push or retag.

## 3. Push the tag (triggers artifacts)

```sh
git push origin stat-tracker-v0.4.10
```

Watch:

- Actions: https://github.com/FrozenTear/scuffed-crew/actions/workflows/stat-tracker-release.yml
- Release: https://github.com/FrozenTear/scuffed-crew/releases/tag/stat-tracker-v0.4.10

The job injects `SST_RELEASE_VERSION=0.4.10` so
`scuffed-stat-tracker --version` is `scuffed-stat-tracker 0.4.10`.

Release notes are `crates/stat-tracker/CHANGELOG.md` section `## 0.4.10`, then
the usual git-log since the previous `stat-tracker-v*` tag (0.4.9), then
...
scuffed-stat-tracker --version   # expect: scuffed-stat-tracker 0.4.10
# POTG nameplate title card (color gate) wakes slow poll to full ~4s cadence for 45s
# Letterbox / highlight end-reel path unchanged; phrase OCR optional / log only
```
...
- Do **not** `gh release create` by hand — the workflow owns the tarball,
  sha256, and notes.
- Do **not** push `stat-tracker-v0.4.10` onto a commit that is not on `main`
  after the merge.
